...
namespace LrcDownloader
{
    internal static class QqDesHelper
    {
        ...
        public static void TripleDESKeySetup(byte[] key, byte[][][] schedule, uint mode)
        {
            if (mode == ENCRYPT)
            {
                KeySchedule(Slice(key, 0), schedule[0], mode);
                KeySchedule(Slice(key, 8), schedule[1], DECRYPT);
                KeySchedule(Slice(key, 16), schedule[2], mode);
            }
            else /*if (mode == DES_DECRYPT*/
            {
                KeySchedule(Slice(key, 0), schedule[2], mode);
                KeySchedule(Slice(key, 8), schedule[1], ENCRYPT);
                KeySchedule(Slice(key, 16), schedule[0], mode);
            }
        }
